...
// WHEN WORKING LOCAL "http://localhost:3000/posts"
...
export const postSlice = createSlice({
  ...
  extraReducers: {
    ...
    [updatePost.fulfilled]: (state, action) => {
      state.value.map((post) => (post._id === state.currentID ? action.payload : post));
      state.currentID = null;
    ...
    [deletePost.fulfilled]: (state, action) => {
      state.value.filter((post) => post._id !== state.currentID);
      state.currentID = null;
      state.refrech = !state.refrech;
    },
